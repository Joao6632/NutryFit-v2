...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const carregarDadosDoRelatorio = () => {
        ...
        if (!relatorio) {
            return mostrarErro(`Relatório com ID "${relatorioId}" não foi encontrado.`);
        }

        const todasAvaliacoes = JSON.parse(localStorage.getItem("nutrifit-avaliacoes") || "[]");
        const avaliacao1 = todasAvaliacoes.find(av => av.id === relatorio.idAvaliacao1);
        const avaliacao2 = todasAvaliacoes.find(av => av.id === relatorio.idAvaliacao2);

        if (!avaliacao1 || !avaliacao2) {
            return mostrarErro("Os dados das avaliações para este relatório não foram encontrados.");
        }
        ...
        // ============================================================================
        let antiga, nova;
        if (new Date(avaliacao1.dataAvaliacao) < new Date(avaliacao2.dataAvaliacao)) {
    ...
    const preencherHeader = (antiga, nova) => {
        ...
        const dataAntiga = new Date(antiga.dataAvaliacao).toLocaleDateString("pt-BR", { timeZone: 'UTC' });
        const dataNova = new Date(nova.dataAvaliacao).toLocaleDateString("pt-BR", { timeZone: 'UTC' });
    ...
    const preencherMetricas = (antiga, nova) => {
        const configs = [
            { el: ".col-md-3:nth-child(1)", campo: "peso", unidade: "kg", decimais: 2, titulo: "Peso" },
            { el: ".col-md-3:nth-child(2)", campo: "percentualGordura", unidade: "%", decimais: 2, titulo: "% de gordura" },
            { el: ".col-md-3:nth-child(3)", campo: "quadril", unidade: "cm", decimais: 0, titulo: "Quadril" },
            { el: ".col-md-3:nth-child(4)", campo: "cintura", unidade: "cm", decimais: 0, titulo: "Cintura" }
        ];
        ...
        configs.forEach(config => {
            ...
            card.querySelector(".metric-value").textContent = `${valorNovo.toLocaleString('pt-BR', { minimumFractionDigits: config.decimais, maximumFractionDigits: config.decimais })} ${config.unidade}`;
            
            const changeEl = card.querySelector(".metric-change");
            const variacaoAbs = Math.abs(variacao).toLocaleString('pt-BR', { minimumFractionDigits: config.decimais, maximumFractionDigits: config.decimais });
            
            changeEl.innerHTML = `
                <i class="bi bi-triangle-fill" style="${variacao < 0 ? 'transform: rotate(180deg);' : ''}"></i>
                ${variacao > 0 ? '+' : ''}${variacao.toLocaleString('pt-BR', { minimumFractionDigits: config.decimais, maximumFractionDigits: config.decimais })} ${config.unidade}
            `;
    ...
    const preencherDetalhes = (antiga, nova) => {
        const configs = [
            ...
            { el: ".detail-item:nth-child(4)", campo: "altura", formatar: (av) => {
                if (av.altura && !isNaN(av.altura)) {
                    return `${parseFloat(av.altura).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} m`;
                }
                return "Não informado";
            } },
            { el: ".detail-item:nth-child(5)", campo: "frequenciaRefeicoes" },
            { el: ".detail-item:nth-child(6)", campo: "usoSubstancias", formatar: (av) => {
                if (av.detalhesSubstancias && av.detalhesSubstancias.trim() !== "") {
                    return av.detalhesSubstancias;
                }
                if (av.alcool || av.drogasIlicitas) {
                    return `Álcool: Sim`;
                }
                return 'Nenhuma';
            }}
        ...
        configs.forEach(config => {
            ...
            item.querySelector(".detail-sublabel").textContent = config.formatar ? config.formatar(nova) : (String(valorNovo) || "Não informado");
    ...
    const preencherObservacoes = (antiga, nova) => {
        ...
        if (cardAntigo) {
            cardAntigo.querySelector(".observacao-title").textContent = `Observações - Aval. #${antiga.numeroAvaliacao}`;
            cardAntigo.querySelector(".observacao-text").textContent = antiga.observacoes || "Nenhuma observação.";
        }
        if (cardNovo) {
            cardNovo.querySelector(".observacao-title").textContent = `Observações - Aval. #${nova.numeroAvaliacao}`;
            cardNovo.querySelector(".observacao-text").textContent = nova.observacoes || "Nenhuma observação.";
        }
    };
